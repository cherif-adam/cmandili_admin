...
export default function GhostMenuClient({
  restaurantId,
  restaurantName,
  isGhost,
  initialItems,
  ...
  async function saveItem() {
    ...
    const res = await fetch("/api/restaurants/menu", {
      ...
      body: JSON.stringify({
        ...(isEdit ? { id: form.id } : { restaurant_id: restaurantId }),
        name: form.name.trim(),
  ...
  async function deleteItem(id: string) {
    ...
    const res = await fetch(`/api/restaurants/menu?id=${id}`, { method: "DELETE" });
    if (res.ok) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">{items.length} plat{items.length !== 1 ? "s" : ""}</p>
          </div>
        </div>
        <button
          onClick={openAdd}
      ...
      {showForm && (
        <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 space-y-4">
          <h3 className="font-semibold text-white">
            {form.id ? "Modifier le plat" : "Nouveau plat"}
          </h3>
          {error && (
      ...
      {Object.keys(byCategory).length === 0 ? (
        <div className="bg-gray-900 border border-gray-800 rounded-xl px-5 py-12 text-center text-gray-500">
          Aucun plat pour l&apos;instant. Cliquez sur &quot;Ajouter un plat&quot; pour commencer.
        </div>
      ) : (
